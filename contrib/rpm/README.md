...
The RPM spec file provided here is for Stock-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://stock.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Stock git repository](https://github.com/stock/stock) but are not part of
...
Currently only `Source10` is used, for the example `stock.conf` file.
...
in the stock git repository. At present only an SVG version of the Stock
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the stock
...
needed when 0.12.1 is released, a proper fix is already in the Stock git
master branch.
...
idea to build Stock Core against a static version of that library compiled
...
The `stock.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `stock-server` package. At this point, most RPM based
...
The `stock-server` package should have SELinux support. How to properly do
...
Stock-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/stock/). He has been
packaging Stock for Fedora at least since 2012.
...
4. I renamed the `stock` package that contains the Qt GUI to `stock-core` as
that appears to be how the general population refers to it, in contrast to
`stock-xt` or `stock-classic`. I wanted to make sure the general population
