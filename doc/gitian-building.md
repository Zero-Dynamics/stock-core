...
*Setup instructions for a Gitian build of Stock Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Stock
...
- [Building Stock Core](#building-stock-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-stock-host/README.md).
...
Clone the git repositories for stock and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/stock/stock-core
```
...
in the stock repository under 'Fetch and create inputs' to install sources which require
...
To build Stock Core (for Linux, OS X and Windows) just follow the steps under 'Setup and perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the stock repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/stock/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/stock/stock-core
...
./bin/gbuild --commit stock-core=${COMMIT} --url stock-core=${URL} ../stock-core/contrib/gitian-descriptors/gitian-arm.yml
./bin/gbuild --commit stock-core=${COMMIT} --url stock-core=${URL} ../stock-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit stock-core=${COMMIT} --url stock-core=${URL} ../stock-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit stock-core=${COMMIT} --url stock-core=${URL} ../stock-core/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the stock git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../stock/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/stock-core/stock-detached-sigs.git

STOCKPATH=/some/root/path/stock
SIGPATH=/some/root/path/stock-detached-sigs

./bin/gbuild --url stock=${STOCKPATH},signature=${SIGPATH} ../stock/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/stock-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/stock-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/stock-osx-build.assert
```
...
[Stock/stock-sigs](https://github.com/stock/stock-sigs) repository, or if that's not possible create a pull
request.
