...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total stock balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            for(const CTxDestination& addr: addresses)
                a.push_back(CStockAddress(addr).ToString());
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"stockaddress\"\n"
            "\nReturn information about the given stock address.\n"
            "\nArguments:\n"
            "1. \"stockaddress\"     (string, required) The stock address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"stockaddress\", (string) The stock address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"stockaddress\", (string) The stock staking address part of a cold staking address\n"
            "  \"spendingaddress\" : \"stockaddress\", (string) The stock spending address part of a cold staking address\n"
            "  \"votingaddress\" : \"stockaddress\", (string) The stock voting address part of a cold staking v2 address\n"
    ...
    if (isValid)
    {
        ...
        if (address.IsColdStakingAddress(Params())) {
            CStockAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CStockAddress spendingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: Stock address and we have full public key:
        CStockAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are stock addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) stock address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CStockAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"stockaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"stockaddress\"  (string, required) The stock address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CStockAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CStockAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CStockAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
bool getAddressesFromParamsForHistory(const UniValue& params, std::vector<std::pair<std::pair<uint160, uint160>, AddressHistoryFilter>> &addresses)
{
    if (params[0].isStr()) {
        ...
        {
            CStockAddress addressVoting, addressStaking, addressSpending;
        ...
        {
            CStockAddress addressStaking, addressSpending;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            if (address.IsColdStakingv2Address(Params()))
            {
                CStockAddress addressSpending, addressVoting, addressStaking;
                uint160 hashBytes, hashBytesSpending, hashBytesVoting, hashBytesStaking;
                int type = 0;
...
UniValue getaddresshistory(const UniValue& params, bool fHelp)
{
    ...
    std::map<CStockAddress, balStruct> balance;
