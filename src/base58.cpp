...
namespace
{
class CStockAddressVisitor : public boost::static_visitor<bool>
{
private:
    CStockAddress* addr;

public:
    CStockAddressVisitor(CStockAddress* addrIn) : addr(addrIn) {}
...
bool CStockAddress::Set(const CKeyID& id)
{
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CStockAddress::Set(const CKeyID& id, const CKeyID& id2)
{
    SetData(Params().Base58Prefix(CChainParams::COLDSTAKING_ADDRESS), &id, 20, &id2, 20);
    return true;
}

bool CStockAddress::Set(const blsctDoublePublicKey &id)
{
...
bool CStockAddress::Set(const CKeyID& id, const CKeyID& id2, const CKeyID& id3)
{
    SetData(Params().Base58Prefix(CChainParams::COLDSTAKING_ADDRESS_V2), &id, 20, &id2, 20, &id3, 20);
    return true;
}

bool CStockAddress::Set(const CScriptID& id)
{
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CStockAddress::Set(const CScript& scriptIn)
{
    SetData(Params().Base58Prefix(CChainParams::RAW_SCRIPT_ADDRESS), &scriptIn[0], scriptIn.size());
    return true;
}

bool CStockAddress::Set(const CTxDestination& dest)
{
    return boost::apply_visitor(CStockAddressVisitor(this), dest);
}

bool CStockAddress::IsValid() const
{
    return IsValid(Params());
}

bool CStockAddress::IsPrivateAddress(const CChainParams& params) const
{
    return vchVersion == params.Base58Prefix(CChainParams::BLS_PRIVATE_ADDRESS);
}

bool CStockAddress::GetSpendingAddress(CStockAddress &address) const
{
...
bool CStockAddress::GetStakingAddress(CStockAddress &address) const
{
    if(!IsColdStakingAddress(Params()) && !IsColdStakingv2Address(Params()))
        return false;
    uint160 id;
...
bool CStockAddress::GetVotingAddress(CStockAddress &address) const
{
...
bool CStockAddress::IsValid(const CChainParams& params) const
{
...
bool CStockAddress::IsColdStakingAddress(const CChainParams& params) const
{
    return vchVersion == params.Base58Prefix(CChainParams::COLDSTAKING_ADDRESS) && vchData.size() == 40;
}


bool CStockAddress::IsColdStakingv2Address(const CChainParams& params) const
{
    return vchVersion == params.Base58Prefix(CChainParams::COLDSTAKING_ADDRESS_V2) && vchData.size() == 60;
}

CTxDestination CStockAddress::Get() const
{
    if (!IsValid())
...
bool CStockAddress::GetIndexKey(uint160& hashBytes, int& type) const
{
    if (!IsValid()) {
        return false;
...
bool CStockAddress::GetKeyID(CKeyID& keyID) const
{
...
bool CStockAddress::GetStakingKeyID(CKeyID& keyID) const
{
...
bool CStockAddress::GetSpendingKeyID(CKeyID& keyID) const
{
    if (!(IsValid() && (vchVersion == Params().Base58Prefix(CChainParams::COLDSTAKING_ADDRESS)
                     || vchVersion == Params().Base58Prefix(CChainParams::COLDSTAKING_ADDRESS_V2))))
        return false;
...
bool CStockAddress::GetVotingKeyID(CKeyID& keyID) const
{
...
bool CStockAddress::IsScript() const
{
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

bool CStockAddress::IsRawScript() const
{
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::RAW_SCRIPT_ADDRESS);
}

void CStockSecret::SetKey(const CKey& vchSecret)
{
...
bool CStockSecret::IsValid() const
{
    if (vchVersion == Params().Base58Prefix(CChainParams::BLS_PRIVATE_ADDRESS))
    {
        uint8_t vData[bls::G1Element::SIZE];
...
bool CStockSecret::SetString(const char* pszSecret)
{
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CStockSecret::SetString(const std::string& strSecret)
{
