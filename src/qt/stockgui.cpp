...
const QString StockGUI::DEFAULT_WALLET = "~Default";

StockGUI::StockGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    fShowingVoting(0)
{
    ...
    for(StockUnits::Unit u: StockUnits::availableUnits())
    {
        unitDisplayControl->addItem(QString(StockUnits::name(u)), u);
    }
...
#ifdef ENABLE_WALLET
    if(enableWallet) {
        connect(walletFrame, &WalletFrame::requestedSyncWarningInfo, this, &StockGUI::showModalOverlay);
        connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &StockGUI::showModalOverlay);
        connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &StockGUI::showModalOverlay);
    }
...
float StockGUI::scale()
{
    return GUIUtil::scale();
}

void StockGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->Icon(":/icons/stock"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Stock address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and stock: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Stock addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->IconAlt(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Stock addresses"));
    ...
    openAction->setStatusTip(tr("Open a stock: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->IconAlt(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Stock command-line options").arg(tr(PACKAGE_NAME)));
...
void StockGUI::createHeaderWidgets()
{
...
void StockGUI::showOutOfSyncWarning(bool fShow)
{
    showHideNotification(fShow, 1);
}

void StockGUI::showHideNotification(bool show, int index)
{
    notifications[index]->setVisible(show);
}

void StockGUI::setActiveMenu(int index)
{
    for (int i = 0; i < 6; ++i)
    {
...
bool StockGUI::checkSettingsSaved()
{
...
void StockGUI::setBalance(const CAmount &avail, const CAmount &pendi, const CAmount &immat, const CAmount& priv, const CAmount& privpending, const CAmount& privlocked)
{
    ...
    balanceAvail->setText(StockUnits::prettyWithUnit(unit, avail, false, StockUnits::separatorAlways));
    balancePendi->setText(StockUnits::prettyWithUnit(unit, pendi, false, StockUnits::separatorAlways));
    balanceImmat->setText(StockUnits::prettyWithUnit(unit, immat, false, StockUnits::separatorAlways));
    privAvail->setText(StockUnits::prettyWithUnit(unit, priv, false, StockUnits::separatorAlways, true));
    privPendi->setText(StockUnits::prettyWithUnit(unit, privpending, false, StockUnits::separatorAlways, true));
}

void StockGUI::setStaked(const CAmount &all, const CAmount &today, const CAmount &week)
{
    if (!walletFrame || !clientModel || !clientModel->getOptionsModel())
        return;
    ...
    stakedAvail->setText(StockUnits::prettyWithUnit(unit, all, false, StockUnits::separatorAlways));
    stakedPendi->setText(StockUnits::prettyWithUnit(unit, today, false, StockUnits::separatorAlways));
    stakedImmat->setText(StockUnits::prettyWithUnit(unit, week, false, StockUnits::separatorAlways));
}

void StockGUI::onDaoEntriesChanged(int count)
{
...
void StockGUI::setMenuBubble(int index, int drak)
{
...
void StockGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool StockGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool StockGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void StockGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void StockGUI::updateAlerts(const QString &warnings)
{
...
void StockGUI::setWalletActionsEnabled(bool enabled)
{
...
void StockGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
...
void StockGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &StockGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void StockGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void StockGUI::macosDockIconActivated()
{
    show();
    activateWindow();
}
...
void StockGUI::optionsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void StockGUI::cfundProposalsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void StockGUI::cfundProposalsOpen(bool fMode)
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void StockGUI::cfundPaymentRequestsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void StockGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void StockGUI::infoClicked()
{
    if(!clientModel)
        return;

    QString link = QString("https://info.stock.org/");
    QDesktopServices::openUrl(QUrl(link));
}

void StockGUI::showDebugWindow()
{
...
void StockGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void StockGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void StockGUI::openClicked()
{
...
void StockGUI::gotoOverviewPage()
{
...
void StockGUI::gotoCommunityFundPage()
{
    if (!checkSettingsSaved())
        return;
...
void StockGUI::gotoSettingsPage()
{
    setActiveMenu(5);
...
void StockGUI::gotoReceiveCoinsPage()
{
    if (!checkSettingsSaved())
        return;
...
void StockGUI::gotoRequestPaymentPage()
{
    if (!checkSettingsSaved())
        return;
...
void StockGUI::gotoSendCoinsPage(QString addr)
{
    if (!checkSettingsSaved())
        return;
...
void StockGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void StockGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void StockGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Stock network", "", count));
}

void StockGUI::updateHeadersSyncProgressLabel()
{
...
void StockGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
#ifdef Q_OS_MAC
...
void StockGUI::showVotingDialog()
{
  ...
  if(showVoting)
  {
    ...
    msgBox.setInformativeText(tr("The Stock Network is currently voting on introducing changes on the consensus protocol. As a participant in our network, we value your input and the decision ultimately is yours. Please cast your vote. <br><br>For more information on the proposal, please visit <a href=\"https://stock.org/community-fund\">this link</a><br><br>Would you like the Stock Network to update the staking rewards to setup a decentralised community fund that will help grow the network?"));
...
void StockGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Stock"); // default title
...
void StockGUI::changeEvent(QEvent *e)
{
...
void StockGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void StockGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void StockGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(StockUnits::formatWithUnit(unit, amount, true, StockUnits::SeparatorStyle::separatorStandard, type.contains("Private Payment"))) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void StockGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void StockGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool StockGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool StockGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void StockGUI::setEncryptionStatus(int status)
{
...
void StockGUI::setEncryptionTxStatus(bool fCrypted)
{
...
void StockGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void StockGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void StockGUI::detectShutdown()
{
...
void StockGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void StockGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void StockGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(StockGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static std::string AskForPin(StockGUI *gui)
{
    std::string ret = "";
...
void StockGUI::askForPin(std::string *ret)
{
    bool ok = false;
...
void SetBalance(StockGUI *gui, const CAmount& total, const CAmount& avail, const CAmount &immat, const CAmount& priv, const CAmount& privpending, const CAmount& privlocked)
{
    // Call our instance method
    gui->setBalance(total, avail, immat, priv, privpending, privlocked);
}

void SetStaked(StockGUI *gui, const CAmount& all, const CAmount& today, const CAmount &week)
{
    // Call our instance method
    gui->setStaked(all, today, week);
}

void StockGUI::subscribeToCoreSignals()
{
...
void StockGUI::unsubscribeFromCoreSignals()
{
...
void StockGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
...
void StockGUI::updateDisplayUnit(int unit)
{
    // Update the list value
    unitDisplayControl->setCurrentText(StockUnits::name(unit));
}

/** Update the display currency **/
void StockGUI::comboBoxChanged(int index)
{
...
void StockGUI::updatePrice()
{
    ...
    std::thread pThread{[this]{
        ...
        try {
            CURL *curl;
            std::string url(
                    "https://api.coingecko.com/api/v3/simple/price?ids=stock&vs_currencies="
                    "BTC,"
                    "EUR,"
                    "USD,"
                    "ARS,"
            ...
            boost::property_tree::ptree json = _json.get_child("stock");
...
size_t StockGUI::priceUdateWriteCallback(void* contents, size_t size, size_t nmemb, void* userp)
{
    ((std::string*) userp)->append((char*) contents, size * nmemb);
    return size * nmemb;
}

size_t StockGUI::priceUdateWriteCallbackHeaders(void* contents, size_t size, size_t nitems, void* userdata)
{
    ((std::string*) userdata)->append((char*) contents, size * nitems);
    return size * nitems;
}

void StockGUI::updateStakingStatus()
{
    updateWeight();
