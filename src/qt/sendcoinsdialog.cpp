...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    processSendCoinsReturn(prepareStatus,
        StockUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee(), fPrivate));
    ...
    if (currentTransaction.fSpendsColdStaking && (!fCoinControl ||
        (fCoinControl && !CStockAddress(CoinControlDialog::coinControl->destChange).IsColdStakingAddress(Params()))))
    {
    ...
    const SendCoinsRecipient &rcp = currentTransaction.recipients.first();
    {
        ...
        QString amount = "<b>" + StockUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount - (rcp.fSubtractFeeFromAmount ? txFee : 0) , false, StockUnits::SeparatorStyle::separatorStandard, false, rcp.isanon);
        amount.append("</b>");
        ...
        if(txFee)
        {
            ...
            questionString.append(StockUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee, false, StockUnits::SeparatorStyle::separatorStandard, false, rcp.isanon));
    ...
    if (model->getOptionsModel()->getDisplayUnit() != StockUnits::ODYNS)
        alternativeUnits.append(StockUnits::formatHtmlWithUnit(StockUnits::ODYNS, totalAmount));

    // Check if we have selected a display unit that is not BTC
    if (model->getOptionsModel()->getDisplayUnit() != StockUnits::BTC)
        alternativeUnits.append(StockUnits::formatHtmlWithUnit(StockUnits::BTC, totalAmount));

    questionString.append(tr("Total Amount %1")
        .arg(StockUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(StockUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee));
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    ...
    CStockAddress addr = CStockAddress(text.toStdString());
    ...
    {
        ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Stock change address"));
