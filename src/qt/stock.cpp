...
{
    return QCoreApplication::translate("stock-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. stock_de.qm (shortcut "de" needs to be defined in stock.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. stock_de_DE.qm (shortcut "de_DE" needs to be defined in stock.qrc)
...
/** Class encapsulating Stock Core startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class StockCore: public QObject
{
    Q_OBJECT
public:
    explicit StockCore(std::string& wordlist, std::string& password);
...
/** Main Stock application object */
class StockApplication: public QApplication
{
    Q_OBJECT
public:
    explicit StockApplication(int &argc, char **argv);
    ~StockApplication();
    ...
    /// Get window identifier of QMainWindow (StockGUI)
...
#include <qt/stock.moc>

StockCore::StockCore(std::string& wordlist, std::string& password):
    QObject(),
    words(wordlist),
    password(password)
{
...
void StockCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(strMiscWarning));
}

void StockCore::initialize()
{
...
void StockCore::restart(QStringList args)
{
...
StockApplication::StockApplication(int &argc, char **argv):
...
void StockApplication::loadTheme()
{
    ...
    // This must be done inside the StockApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    platformStyle = PlatformStyle::instantiate();
    assert(platformStyle);
}

void StockApplication::createOptionsModel(bool resetSettings)
{
    optionsModel = new OptionsModel(NULL, resetSettings);
}

// this will be used to get mnemonic words
bool StockApplication::setupMnemonicWords(std::string& wordlist, std::string& password) {
...
bool StockApplication::createWindow(const NetworkStyle *networkStyle)
{
    if (!setupMnemonicWords(wordlist, password)) {
        if (wordlist.empty()) return false;
    }

    window = new StockGUI(platformStyle, networkStyle, 0);
...
void StockApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
...
void StockApplication::startThread()
{
    if(coreThread)
        return;
    coreThread = new QThread(this);
    StockCore *executor = new StockCore(wordlist, password);
...
void StockApplication::parameterSetup()
{
    InitLogging();
    InitParameterInteraction();
}

void StockApplication::requestInitialize()
{
...
void StockApplication::requestShutdown()
{
...
void StockApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        if(pwalletMain)
        {
            ...
            window->addWallet(StockGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(StockGUI::DEFAULT_WALLET);
        }
#endif
...
void StockApplication::shutdownResult(int retval)
{
    qDebug() << __func__ << ": Shutdown result: " << retval;
    quit(); // Exit main loop after shutdown finished
}

void StockApplication::handleRunawayException(const QString &message)
{
    QMessageBox::critical(0, "Runaway exception", StockGUI::tr("A fatal error occurred. Stock can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}

WId StockApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(stock);
    Q_INIT_RESOURCE(stock_locale);

    // Load the app
    StockApplication app(argc, argv);
    ...
    /// 5. Determine availability of data directory and parse stock.conf
