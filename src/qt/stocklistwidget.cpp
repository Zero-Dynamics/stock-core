...
StockListWidget::StockListWidget(QWidget *parent, QString title, ValidatorFunc validator) :
    QWidget(parent),
...
void StockListWidget::onRemove()
{
    listWidget->takeItem(listWidget->row(listWidget->currentItem()));
}

void StockListWidget::onSelect(QListWidgetItem* item)
{
    removeBtn->setVisible(item != nullptr);
}

void StockListWidget::onInsert()
{
...
QStringList StockListWidget::getEntries()
{
    QStringList ret;
