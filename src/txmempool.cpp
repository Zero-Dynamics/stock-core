...
void CTxMemPool::check(const CStateViewCache *pcoins) const
{
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        if (fDependsWait)
            ...
            assert(CheckInputs(tx, state, mempoolDuplicate, false, 0, false, blsctData, txdata, fXOdynSSer, nullptr));
    ...
    while (!waitingOnDependants.empty()) {
        ...
        if (!mempoolDuplicate.HaveInputs(entry->GetTx())) {
            ...
            assert(CheckInputs(entry->GetTx(), state, mempoolDuplicate, false, 0, false, blsctData, txdata, fXOdynSSer, nullptr));
